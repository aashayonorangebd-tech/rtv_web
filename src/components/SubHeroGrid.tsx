...
export default function SubHeroGrid({ stories }: { stories: StoryModel[] }) {
  ...
  return (
    <section className="border-t border-[#dddddd] dark:border-gray-800 pt-6 mt-6 md:px-25">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
        <div className="md:col-span-1 md:border-r border-[#dddddd] dark:border-gray-800 md:pr-6 pb-6 md:pb-0">
          {col1.map((story) => (
            <StoryCardMedium key={story.storyId} story={story} />
          ))}
        </div>

        <div className="md:col-span-1 md:border-r border-[#dddddd] dark:border-gray-800 md:px-6 pb-6 md:pb-0">
          {col2.map((story) => (
            <StoryCardMedium key={story.storyId} story={story} />
          ))}
        </div>
