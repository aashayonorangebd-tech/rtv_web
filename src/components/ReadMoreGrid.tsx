...
export default function ReadMoreGrid({ title = "আরও পড়ুন", stories = [] }: Props) {
  ...
  return (
    <div className="col-span-full max-sm:mx-2 mt-8 border-t pt-6 dark:border-border">
          ...
          <div key={rmStory.storyId ?? `readmore-${index}`} className="h-full">
            ...
            >
              ...
              <div className="flex flex-col flex-grow w-full">
                <h3 className="text-[18px] font-bold text-gray-800 dark:text-white leading-[26px] group-hover:text-[#005adf] transition-colors line-clamp-4">
                  {rmStory.mainTitle}
                </h3>

                {typeof rmStory.subTitle === "string" && rmStory.subTitle.trim() && (
                  <p className="text-[16px] text-gray-700 dark:text-foreground mt-1 mb-1 leading-[24px] line-clamp-2">
                    {rmStory.subTitle.trim()}
                  </p>
                )}

                <div className="flex items-center text-[14px] text-[#787C85] dark:text-foreground">
